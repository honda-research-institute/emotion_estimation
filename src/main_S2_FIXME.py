...
#################################   
# 1) Read S2's calibration data - obtain the mean and std for each signal to Standardize the downstream task
#################################
calibration_data, scaler_dict = read_individual_hri_dataset("data/Hug_exp/S2/calibration", "data/Hug_exp/S2_calib.h5", config['hri']['percent_overlap'], config, save=True, standardize=True, input_scaler=None, calib=True)
# plt.plot(calibration_data['S2']['event']['labels'][:, 1], calibration_data['S2']['event']['labels'][:, 0], '.')

# Extract SSL_features
if  not os.path.isfile("data/Hug_exp/S2_calib_feats.h5"):
    ...
    ECG_data = EcgDataset("data/Hug_exp/S2_calib.h5", window_len, data_group=str('/data/event'))
    ...
    dd.io.save("data/Hug_exp/S2_calib_feats.h5", Multi_features)

balance_features = False
features = dd.io.load("data/Hug_exp/S2_calib_feats.h5")
...
#############################
# Read S2's experiment Human robot experiment data 
#############################
exp_data, _ = read_individual_hri_dataset("data/Hug_exp/S2/approach3", "data/Hug_exp/S2_exp.h5", config['hri']['percent_overlap'], config, save=True, standardize=False, input_scaler=scaler_dict)

# Extract SSL_features
if  not os.path.isfile("data/Hug_exp/S2_exp_feats.h5"):
    ckpt_file  = 'model2_1/net_200.pth'
    checkpoint = torch.load(config['torch']['SSL_models'] + ckpt_file, map_location=device)
    net = SelfSupervisedNetFeats(load_model=True, checkpoint=checkpoint, device=device, config=config).to(device)

    ECG_data = EcgDataset("data/Hug_exp/S2_exp.h5", window_len, data_group=str('/data/event'))
    SSL_dataloader = torch.utils.data.DataLoader(ECG_data, batch_size=batch_size, shuffle=False)
    SSL_features, labels = net.return_SSL_feats(SSL_dataloader)
    ...
    dd.io.save("data/Hug_exp/S2_exp_feats.h5", Multi_features)

balance_features = False
features_exp = dd.io.load("data/Hug_exp/S2_exp_feats.h5")
...
if not os.path.isfile("data/Hug_exp/RF_S2.pkl"):
    regressor = (RandomForestRegressor(n_estimators=1000, max_depth=30))
    regressor = train_regression_model('', train_dataset, regressor, clean_features=True, model_save_path="data/Hug_exp/RF_S2.pkl")
else:
    regressor = pickle.load(open("data/Hug_exp/RF_S2.pkl", 'rb'))
